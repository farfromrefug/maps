...
L.PhotonBase = L.Class.extend({
  ...
  featureToPopupContent: function (feature) {
    const container = L.DomUtil.create('div', 'leaflet-photon-popup')
...
L.PhotonBaseSearch = L.PhotonBase.extend({
  includes:
    (typeof L.Evented !== 'undefined' && L.Evented.prototype) || L.Mixin.Events,
  ...
  createResult: function (feature) {
    const el = L.DomUtil.create('li', '', this.resultsContainer)
  ...
  handleResults: function (geojson) {
    ...
    if (geojson.features.length === 0) {
      const noresult = L.DomUtil.create(
        'li',
        'photon-no-result',
        this.resultsContainer
      )
    ...
    if (this.options.feedbackEmail) {
      const feedback = L.DomUtil.create(
        'a',
        'photon-feedback',
        this.resultsContainer
      )
...
L.PhotonSearch = L.PhotonBaseSearch.extend({
  ...
  getParams: function () {
    const params = L.PhotonBaseSearch.prototype.getParams.call(this)
...
L.Map.addInitHook(function () {
  if (this.options.photonControl) {
    this.photonControl = new L.Control.Photon(
      this.options.photonControlOptions || {}
    )
